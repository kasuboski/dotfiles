...
```bash
nix run github:nix-community/nixos-anywhere -- --phases kexec,disko --generate-hardware-config nixos-generate-config ./hosts/forge/hardware-configuration.nix --flake .#forge --target-host root@nixos.lan
```
...
```bash
nix run github:nix-community/nixos-anywhere -- --phases install,reboot --generate-hardware-config nixos-generate-config ./hosts/forge/hardware-configuration.nix --flake .#forge --target-host root@<ip>
```
